...
function drawNote(ctx, location, duration=0.5,color="black") {
	ctx.globalAlpha=Math.min(1,Math.abs(location.x-MARGIN_LEFT)/MARGIN_LEFT);
...
function updateMovingNotes() {
    for (let i = 0; i < MOVING_NOTES.length; i++) {
        ...
        if (MOVING_NOTES[i].location.x <= MARGIN_LEFT) {
			...
			highlightClef(MOVING_NOTES[i].duration,
			MOVING_NOTES[i].isAuto);
...
class Clef{
	constructor(){
		...
		this.assets.normalImage.src="https://radufromfinland.com/projects/melodymaker/clef_glow_white.png";
		this.assets.autoImage.src="https://radufromfinland.com/projects/melodymaker/clef_glow_purple.png";
		this.assets.activeImage.src="https://radufromfinland.com/projects/melodymaker/clef_glow_blue.png";
	...
	draw(ctx){
		let aspectRatio = this.assets.normalImage.width / this.assets.normalImage.height;
		let newHeight = CANVAS.height * 0.65;
		let newWidth = aspectRatio * newHeight;
		ctx.drawImage(this.assets.normalImage,
